...
function related_case_results_shortcode($atts, $content = '') {
    $atts = shortcode_atts(array(
        ...
        'taxonomy' => 'post_tag',
        'tag_slug' => '',
        'ids' => '', // ✅ New parameter for specific post IDs
        'item_bg_color' => '',
        'title_color' => '',
        'excerpt_color' => '',
    ...
    if (!empty($atts['ids'])) {
        ...
        $args['post__in'] = $ids;
        $args['orderby'] = 'post__in'; // preserve given order
    }
    // ✅ Otherwise filter by tag if available
...
function display_case_results_cta_meta_box($post) {
    $shortcode = '[related_case_results title="Recent Case Results" posts="2" columns="2" background="" button_text="View All Case Results" button_link="#" taxonomy="post_tag" tag_slug="your-tag-slug" item_bg_color="#f7f7f7" title_color="#fff" excerpt_color="#555" ids="526,533"]';
...
function related_insights_shortcode($atts) {
    ...
    $padding = !empty($atts['padding']) ? esc_attr($atts['padding']) : '';
    ...
    if ($related_posts->have_posts()) :
        ...
        <section class="widget widget_related_insights">
            <?php if (!empty($atts['title'])) : ?>
                 <h4 class="in-widget-title"><?php echo esc_html($atts['title']); ?></h4>
            <?php endif; ?>
                    ...
                    <article id="post-<?php the_ID(); ?>" <?php post_class('posts-list'); ?>>
                        <div class="posts-thumbnail">
                            <a href="<?php the_permalink(); ?>">
                                ...
                                <?php else : ?>
                                    <img src="<?php echo esc_url(get_stylesheet_directory_uri() . '/images/default-post1.webp'); ?>" alt="<?php the_title_attribute(); ?>" width="398" height="241">
                                <?php endif; ?>
                            </a>
                        </div>
...
function display_related_insights_cta_meta_box($post) {
    $shortcode = '[related_insights posts="2" columns="2" post_type="post" title="Related Insights" taxonomy="post_tag" tag_slug="" post_ids="451,158" heading_color="#fff" text_color="" bg_color="#2b3346"]';
    ...
    echo '<p style="font-size: 12px; margin-top: 5px;">
        ...
        <b>Options:</b><br>
        • <code>posts="3"</code> → Number of posts to show.<br>
...
function team_member_shortcode($atts) {
    ...
    if ($team_query->have_posts()) :
        ...
        <section id="<?php echo esc_attr($unique_id); ?>" class="widget widget_team_member">
            <?php if (!empty($atts['title'])) : ?>
                <div class="team-cta-title"><h4><?php echo esc_html($atts['title']); ?></h4></div>
            <?php endif; ?>
    ...
    <a href="<?php the_permalink(); ?>">
        ...
        <?php } else { ?>
            <img src="<?php echo esc_url( get_stylesheet_directory_uri() . '/images/team-member1.webp' ); ?>" 
        ...
        <?php } ?>
                        ...
                        <div class="team_member_name">
                            ...
                            <?php if (get_field('single_profile_designation')) { ?>
                                <p class="team_designation"><?php echo esc_html(get_field('single_profile_designation')); ?></p>
                            <?php } ?>
                        </div>
                    </div>
                <?php endwhile; ?>
...
function display_team_member_cta_meta_box($post) {
    $shortcode = '[team_member posts="2" columns="2" taxonomy="post_tag" tag_slug="senior-attorneys" title="Meet Our Attorneys" button_text="See More Attorneys" button_link="#" border_color="#ddd" hover_color="#000" name_color="#000" summary_color="#333" post_ids="303,1039"]';
...
/** Testimonials CTA Shortcode (Dynamic Platforms + Specific IDs) */
function testimonial_cta_box($atts) {
    $atts = shortcode_atts(array(
        'title' => '',
        'posts' => 3,
        'ids' => '', // ✅ new: specific post IDs
        'background' => '', // unused
        ...
        'heading_color' => '',
        'background_color' => '',
        'review_text' => 'View More Reviews on', 
        'platforms' => '', // ✅ new dynamic platforms
    ...
    // If specific IDs are given, use them instead of taxonomy
    if (!empty($atts['ids'])) {
    ...
    if ($testimonial_query->have_posts()) :
        ...
        <section class="testimonial-cta-block">
            ...
            <div class="container">
                ...
                <div class="testi-slider-list" style="<?php echo!empty($atts['background_color']) ? 'background-color:' . esc_attr($atts['background_color']) . ';' : ''; ?>">
                                ...
                                <div class="testi-content">
                                    ...
                                    <div class="star-rating-list">
                                        <?php if (get_field('where_from_review_logo')) { ?>
                                            <div class="where-from-review-logo">
                                                <a <?php if (get_field('where_from_review_link')) { ?> href="<?php echo esc_url(get_field('where_from_review_link')); ?>" target="_blank" <?php } ?>>
                                                    <img src="<?php echo esc_url(get_field('where_from_review_logo')); ?>" alt="where from review image">
                                                </a>
                                            </div>
                                        <?php } ?>
                                    <h6 class="testi-name" style="<?php echo!empty($atts['heading_color']) ? 'color:' . esc_attr($atts['heading_color']) . ';' : ''; ?>">
                                        ...
                                        <div class="star-rating">
                                            <img src="<?php echo esc_url(home_url('/wp-content/themes/guzmanandtokar/images/sdbr-start-img.webp')); ?>" alt="">
                                        </div>
                    ...
                    <?php if (!empty($atts['platforms'])) : ?>
                        <p class="review-links">
                            <?php echo esc_html($atts['review_text']); ?> 
                            <?php
                            $platform_pairs = explode(',', $atts['platforms']);
                            $output_links = array();

                            foreach ($platform_pairs as $pair) {
                                $parts = array_map('trim', explode('|', $pair));
                                if (count($parts) == 2 && !empty($parts[0]) && !empty($parts[1])) {
                                    $output_links[] = '<a href="' . esc_url($parts[1]) . '" target="_blank">' . esc_html($parts[0]) . '</a>';
                                }
                            }

                            $count = count($output_links);
                            if ($count > 1) {
                                echo implode(', ', array_slice($output_links, 0, -1)) . ' and ' . end($output_links);
                            } elseif ($count === 1) {
                                echo $output_links[0];
                            }
                            ?>
...
/** Testimonial CTA Meta Box */
function add_testimonial_cta_meta_box() {
    add_meta_box(
            'testimonial_cta_shortcode_box',
            'Testimonial CTA Shortcode',
            'display_testimonial_cta_meta_box',
            'page',
            'side',
            'high'
    );
...
function display_testimonial_cta_meta_box($post) {
    $shortcode = '[testimonial_block title="Client Testimonials" posts="3" text_color="#333" ids="1059,1058,1056" heading_color="#000" background_color="#fafafa" button_text="See All Reviews" button_link="" review_text="Check Reviews on" platforms="Google Maps|https://goo.gl/maps/example, Yelp|https://yelp.com/example, Facebook|https://facebook.com/example, Avvo|https://www.avvo.com/example" ]';

    echo '<textarea readonly style="width:100%; height:160px;" onclick="this.select();">' . esc_textarea($shortcode) . '</textarea>';

    echo '<p> Copy this shortcode and paste it where you want the testimonial CTA block to appear.<br><br> <strong>Optional:</strong> Use <code>text_color</code>, <code>heading_color</code>, or <code>background_color</code> to adjust colors.<br><br> <strong>New:</strong> Add multiple review platforms using the <code>platforms</code> attribute.<br> Example format:<br> <code>platforms="Google Maps|https://goo.gl/maps/example, Yelp|https://yelp.com/example, Facebook|https://facebook.com/example"</code><br><br> Each platform must be written as:<br> <code>Name|URL</code>, separated by commas. </p>'; }
